...
namespace Integer.Domain.Agenda
{
                                         ...
                                         DomainEventHandler<HorarioDeEventoAlteradoEvent>
    {
        ...
        private void RemoverConflitosDeReservaDeLocalDeOutrosEventos(Evento eventoAlterado)
        {
            ...
            {
                bool possuiConflito = eventoComConflito.VerificarSeReservouLocais(eventoAlterado.Reservas);
                if (!possuiConflito)
        ...
        private void RemoverConflitosDeReservaDeLocalDoEvento(Evento eventoAlterado)
        {
            ...
            if (conflitosDeLocal.Count() > 0) 
            {
                ...
                {
                    bool possuiConflito = eventoConflitante.VerificarSeReservouLocais(eventoAlterado.Reservas);
                    if (!possuiConflito)
        ...
        {
            IEnumerable<Evento> eventosComConflito = eventos.QuePossuemConflitoCom(eventoAlterado, MotivoConflitoEnum.ExisteEventoPrioritarioNaData);
        ...
        {
            IEnumerable<Conflito> conflitosComEventosParoquiais = eventoAlterado.Conflitos.Where(c => c.Motivo == MotivoConflitoEnum.ExisteEventoPrioritarioNaData);
